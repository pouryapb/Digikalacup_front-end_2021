...
game = {
  ...
  handleDragAndDrop: function () {
    ...
    game.darkerBox.addEventListener("dragover", (e) => {
      e.preventDefault();
    });
    game.lighterBox.addEventListener("dragover", (e) => {
      e.preventDefault();
    });
    game.darkerBox.addEventListener("drop", (e) => {
      let data = e.dataTransfer.getData("text/plain");
      let draggedElement = document.getElementById(data);
      draggedElement.classList.add("hidden");
      game.darkerBox.append(draggedElement);
      if (game.optionsBox.children.length == 0) {
        game.submitButton.disabled = false;
      }
    });
    game.lighterBox.addEventListener("drop", (e) => {
      let data = e.dataTransfer.getData("text/plain");
      let draggedElement = document.getElementById(data);
      draggedElement.classList.add("hidden");
      game.lighterBox.append(draggedElement);
      if (game.optionsBox.children.length == 0) {
        game.submitButton.disabled = false;
      }
    });
